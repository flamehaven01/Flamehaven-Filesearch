...
class TestVectorQuantization(unittest.TestCase):
    ...
    def test_vector_storage_quantized(self):
        ...
        self.assertEqual(
            stored_vector.dtype, np.int8, "Vector should be stored as int8"
        )
        self.assertEqual(
            stored_vector.shape, (384,), "Vector dimension should be preserved"
        )
    ...
    def test_vector_storage_unquantized(self):
        ...
        stored_vector = grid._vector_essences[0]
        self.assertEqual(
            stored_vector.dtype, np.float32, "Vector should be stored as float32"
        )
    ...
    def test_memory_reduction(self):
        ...
        self.assertGreater(
            reduction_ratio, 0.70, "Should reduce memory by at least 70%"
        )
    ...
    def test_search_accuracy_preservation(self):
        ...
        for i in range(50):
            ...
            vec /= np.linalg.norm(vec)
            vectors.append(vec)
            grid_q.inject_essence(
                f"file_{i}.py", {"id": i, "path": f"file_{i}.py"}, vec
            )
            grid_uq.inject_essence(
                f"file_{i}.py", {"id": i, "path": f"file_{i}.py"}, vec
            )
