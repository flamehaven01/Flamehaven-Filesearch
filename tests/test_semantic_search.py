...
class TestEmbeddingGenerator:
    ...
    def test_text_attuning(self):
        ...
        test_cases = [
            ...
            ("HELLO   WORLD", "hello world"),
            ("A" * 600 + "more", "a" * 512), # Max length check (512 is MAX_TEXT_LENGTH)
            ("  spaces  ", "spaces"),
        ]
...
class TestChronosGridIntegration:
    ...
    def test_inject_and_seek_with_essence(self):
        ...
        if ChronosGrid._NUMPY_AVAILABLE_AT_INIT: # Check actual numpy availability at ChronosGrid init
...
@pytest.mark.fast$([char]13)$([char]10)class TestIntentRefinerIntegration:
    ...
    def test_query_correction_integration(self):
        ...
        with patch('flamehaven_filesearch.core.IntentRefiner', return_value=mock_intent_refiner_instance):
            searcher = FlamehavenFileSearch(allow_offline=True)
            
            # The search method will now use the mocked IntentRefiner
            result = searcher.search(
                query="find pythn script",
                search_mode="keyword"
            )
            
            # Assert that the searcher called refine_intent on the mock
            mock_intent_refiner_instance.refine_intent.assert_called_once_with("find pythn script")
    ...
    def test_keyword_extraction(self):
        ...
        with patch('flamehaven_filesearch.core.IntentRefiner', return_value=mock_intent_refiner_instance):
            searcher = FlamehavenFileSearch(allow_offline=True)
            result = searcher.search(
                query="find python scripts for data analysis",
                search_mode="keyword"
            )
            assert "python" in result.get("search_intent", {}).get("keywords", [])


@pytest.mark.fast$([char]13)$([char]10)class TestCoreSemanticSearch:
    ...
    async def test_upload_generates_embedding(self, tmp_path):
        ...
        # Check embedding was generated (by checking cache stats for the shared generator)
        embedding_stats = searcher.embedding_generator.get_cache_stats()
        assert embedding_stats['total_queries'] > 0 # At least one query for "test content"
...
@pytest.mark.fast$([char]13)$([char]10)class TestAPIIntegration:
    ...
    async def test_metrics_endpoint_includes_new_engine_stats(self, client, api_key):
        """Verify /metrics endpoint includes Chronos, IntentRefiner, Gravitas, Embedding stats"""
...
@pytest.mark.fast$([char]13)$([char]10)class TestEndToEndSemanticSearch:
    ...
    def test_complete_workflow(self, tmp_path):
        ...
        test_file_content = "This is a report about Q1 earnings and some python code examples."
        ...
        # Verify that the uploaded file is among the semantic results
        found_semantic = False
        for sr in search_result["semantic_results"]:
            # Mocked semantic results might not contain file_name, check structure
            if isinstance(sr, tuple) and len(sr) == 2 and isinstance(sr[0], dict) and sr[0].get("file") == "mock.txt": # Check mock behavior
                found_semantic = True
                break
            elif isinstance(sr, dict) and sr.get("file_name") == test_file_name: # Actual Chronos-Grid result
                found_semantic = True
                break
        
        # The mock currently returns a generic result; the true test for semantic content depends on ChronosGrid and embedding_generator
        # For this test, we assume ChronosGrid will return results matching the mocked intent_refiner's output structure
        # If we had actual numpy installed and ChronosGrid was using real embeddings, this would be more precise.
        
        # To make this assert more robust for mocked ChronosGrid:
        # Check if the mock was called with the correct embedding and ensure its return value is used.
        ...
        assert len(search_result["semantic_results"]) > 0, "Semantic results should not be empty"
        
        # 4. Search with hybrid mode for a different query
        hybrid_query = "python script for financial analysis"
        hybrid_result = searcher.search(
            query=hybrid_query,
            search_mode="hybrid"
        )
        ...
        corrected_result = searcher.search(
            query=typo_query,
            search_mode="keyword"
        )
