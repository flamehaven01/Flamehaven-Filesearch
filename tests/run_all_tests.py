...
def discover_tests(start_dir='tests', pattern='test_*.py'):
    ...
    try:
        from tests.test_gravitas_cache_integration import TestGravitasPackCacheIntegration
        phase3_tests = loader.loadTestsFromTestCase(TestGravitasPackCacheIntegration)
        suite.addTests(phase3_tests)
        print(f"[+] Loaded: test_gravitas_cache_integration.py ({phase3_tests.countTestCases()} tests)")
    except Exception as e:
